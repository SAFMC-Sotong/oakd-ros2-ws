...
void printStatistics(const Estimator &estimator, double t)
{
    ...
    if (ESTIMATE_TD)
        ROS_INFO("time offset between camera and imu= %f", estimator.td);
}
...
void pubOdometry(const Estimator &estimator, const std_msgs::msg::Header &header)
{
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        odometry.header.frame_id = ODOM_FRAME_ID;
        odometry.child_frame_id = ODOM_CHILD_FRAME_ID;
        Quaterniond tmp_Q;
        ...
        // send tf to base_link
        geometry_msgs::msg::TransformStamped transform;
        transform.header = header;
        transform.header.frame_id = ODOM_FRAME_ID;
        transform.child_frame_id = ODOM_CHILD_FRAME_ID;
        transform.transform.translation.x = estimator.Ps[WINDOW_SIZE].x();
        transform.transform.translation.y = estimator.Ps[WINDOW_SIZE].y();
        transform.transform.translation.z = estimator.Ps[WINDOW_SIZE].z();
        transform.transform.rotation.x = tmp_Q.x();
        transform.transform.rotation.y = tmp_Q.y();
        transform.transform.rotation.z = tmp_Q.z();
        transform.transform.rotation.w = tmp_Q.w();
...
void pubKeyPoses(const Estimator &estimator, const std_msgs::msg::Header &header)
{
    ...
    key_poses.header.frame_id = ODOM_FRAME_ID;
...
//     // br->sendTransform(tf2::StampedTransform(transform, header.stamp, ODOM_FRAME_ID, BODY_FRAME_ID));
//     br->sendTransform(tf2::StampedTransform(transform, header.stamp, ODOM_FRAME_ID, BODY_FRAME_ID));
...
//     q.setZ(Quaterniond(estimator.ric[0]).z());
//     transform.setRotation(q);
//     // br->sendTransform(tf2::StampedTransform(transform, header.stamp, BODY_FRAME_ID, "camera"));
//     br->sendTransform(tf2::StampedTransform(transform, header.stamp, BODY_FRAME_ID, "camera"));
...
void pubKeyframe(const Estimator &estimator)
{
    ...
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR && estimator.marginalization_flag == 0)
    {
        ...
        point_cloud.header.frame_id = ODOM_FRAME_ID;
        for (auto &it_per_id : estimator.f_manager.feature)
        {
